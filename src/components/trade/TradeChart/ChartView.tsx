...
export const ChartView = ({
  ...
  const setMarkerData = () => {
    ...
    const formattedMarkers = createMarkers(userFills);
  ...
  const setOrderData = () => {
    priceLines.forEach((priceLine) => {
      ...
      const temp = priceLines.filter((line) => line !== priceLine);
      setPriceLines(temp);
    })
    if(!chartSetting.trade.showOrders) return;

    const createOrders = (os: Array<any>) => {
      const formatted: Array<UserOrder> = [];
      for (const key in os){
        const o = os[key];
    ...
    const formattedOrders = createOrders(orders);
