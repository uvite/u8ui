...
const renderErrorMessage = (dom: HTMLInputElement, errorMsg: string) => {
	...
	if (!inputRef.parentElement!.querySelector('.workflow__vertex__input__validation')) {
		...
		const msgDom = inputRef.parentElement!.querySelector<HTMLDivElement>(
			'.workflow__vertex__input__validation',
		);
		msgDom!.innerText = errorMsg;
	}
};
...
function Workflow({ current }: molecule.model.IEditor) {
	...
	// 在某位置插入节点
	const handleDrop: IContainerProps<IWorkflowData>['onDropWidgets'] = (
		node,
		graph,
		target,
		x,
	...
	const toggleEditInputDom = (dom: HTMLInputElement, domVisible: boolean) => {
		...
		if (domVisible) {
			...
			(vertexInput.parentElement!.nextElementSibling as HTMLDivElement).style.display =
				'none';
			vertexInput.focus();
	...
	const handleContextMenu = (
		...
		return [
			...
			},
			{
				title: '编辑名称',
				callback: () => {
					const vertexInput = document.querySelector<HTMLInputElement>(
						`.${editingInputKey}--${cell.id}`,
					);
					if (vertexInput) {
										...
										api.delOfflineTask({ taskId: cell.value.id }).then(
											(res) => {
												if (res.code === 1) {
													...
													if (isOpened) {
														const groupId =
															molecule.editor.getGroupIdByTab(id);
														if (groupId) {
		...
		if (current?.tab?.id && viewStoreService.getViewStorage(current?.tab?.id.toString())) {
			...
			try {
				const nodeMap: Record<string, number[]> =
					JSON.parse(current?.tab?.data.sqlText) || {};
