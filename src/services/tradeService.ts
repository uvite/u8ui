	...
	implements IDataSourceService
{
    ...
    api.getTrade(params).then((res) => {
      if (res.code === 1) {
        const nextData: ITradeProps[] = ((res.data as ITradeProps[]) || []).map(
          (ele) => {
            return ele
          },
        );
